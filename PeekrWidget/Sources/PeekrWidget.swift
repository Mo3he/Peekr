...
struct ServiceEntityQuery: EntityQuery {
    ...
    private func allEntities() -> [ServiceEntity] {
        let ud = UserDefaults(suiteName: "group.net.mohome.peekr") ?? .standard
...
struct PeekrWidgetProvider: AppIntentTimelineProvider {
...
private func loadMetrics(for serviceId: String) -> [WidgetMetric] {
    guard let dir = FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: "group.net.mohome.peekr"),
...
struct PeekrOverviewWidget: Widget {
    ...
    var body: some WidgetConfiguration {
        AppIntentConfiguration(kind: kind, intent: OverviewWidgetIntent.self, provider: PeekrWidgetProvider()) { entry in
...
struct PeekrMonitorWidget: Widget {
    let kind = "PeekrMonitorWidget"
    var body: some WidgetConfiguration {
        ...
        .configurationDisplayName("Peekr Monitor")
